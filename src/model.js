...
export class ArchModel {
	...
	static toXml(arch) {
		...
		if (arch.legend?.length) {
			...
			arch.legend.forEach(e => {
				xml += `    <entry id="${e.id}" name="${e.name}" color="${e.color}" />\n`;
			});
			xml += '  </legend>\n';
		}
	...
	static _nodeToXml(node, indent) {
		...
		if (node.children?.length) {
			...
			node.children.forEach(c => {
				xml += this._nodeToXml(c, indent + 2);
			});
	...
	static _parseNode(el) {
		...
		if (posAttr) {
			...
			if (parts.length === 3 && parts.every(p => !Number.isNaN(p))) node.pos = parts;
		}
		for (let i = 0; i < el.childNodes.length; i++) {
			const child = el.childNodes[i];
			if (child.nodeType === 1 && child.tagName.toLowerCase() === 'node') node.children.push(this._parseNode(child));
		}
		return node;
	}
	static createEmpty() {
		...
		model.emitSelect = (id) => {
			model._selectListeners.forEach(fn => {
				try {
					...
					console.error('select listener error', e);
				}
			});
		};
