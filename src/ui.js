...
export class UI {
	...
	_setButtonsVisibility(ids, show) {
		ids.forEach(id => this.setVisibility(id, !!show));
	}
	addMessage(text) {
...
`;
		...
		list.querySelectorAll('.legend-swatch').forEach(swatch => {
			const listener = () => {
				...
				this.openColorPicker(swatch, this.app.model.legend[i].color, (color) => {
					...
					this.updateLegendDisplay();
					if (this.app.model && typeof this.app.model.update === 'function') this.app.model.update(() => {}, {
						type: 'legend-changed'
					}, {
						debounceMs: 60
					});
					else if (this.app.model && typeof this.app.model.emitChange === 'function') this.app.model.emitChange({
						type: 'legend-changed'
					});
				});
			};
	...
	renderLegendAssignSelect() {
		...
		const listener = () => {
			...
			if (!node) return;
			if (sel.value) node.category = sel.value;
			else delete node.category;
			if (this.app.model && typeof this.app.model.emitChange === 'function') this.app.model.emitChange({
				type: 'legend-changed'
			});
		};
	...
	populateEditPanel() {
		...
		if (!this.editMode || !sceneNode) {
			this.setVisibility(panel || 'editPanel', false);
			return;
		}
...
`;
		...
		this.renderLegendAssignSelect();
		const sel = document.getElementById('assignLegendSelect');
		if (sel && archNode?.category) sel.value = archNode.category;
	}
		...
		if (addBtn) {
			...
			const addListener = (e) => {
				...
				const newNode = {
					...
					name: 'New Node',
					pos: [archNode.pos[0], archNode.pos[1] - 2, archNode.pos[2]],
					scale: 1,
					children: []
				};
	...
	setVisibility(elOrId, visible, opts = {
		useHiddenClass: true,
		setAria: true
	}) {
		let el = null;
		...
		if (!el) return;
		if (opts.className) el.classList.toggle(opts.className, !!visible);
		else if (opts.useHiddenClass) el.classList.toggle('hidden', !visible);
		else {
			if (!visible) el.style.display = 'none';
			else el.style.display = (opts.display || 'block');
		}
		if (opts.setAria) {
