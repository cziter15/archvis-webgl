...
export class InputHandler {
  constructor(app, renderer) {
    ...
    this.mobile = { leftTouchId: null, rightTouchId: null, leftStart: null, rightStart: null, leftPos: { x: 0, y: 0 }, rightPos: { x: 0, y: 0 }, maxRadius: 44, upPressed: false, downPressed: false };
  ...
  _setupTouchControls() {
    ...
    document.addEventListener('touchend', (e) => { if (e.touches.length < 2) this.pinchStartDist = null; }, { passive: true });
  }
  ...
  _setupMobileButtons() {
    ...
    if (btnUp) {
      const downHandler = (ev) => { this.mobile.upPressed = true; btnUp.classList.add('active'); ev.preventDefault?.(); };
      const upHandler = (ev) => { this.mobile.upPressed = false; btnUp.classList.remove('active'); ev.preventDefault?.(); };
    ...
    if (btnDown) {
      const downHandlerD = (ev) => { this.mobile.downPressed = true; btnDown.classList.add('active'); ev.preventDefault?.(); };
      const upHandlerD = (ev) => { this.mobile.downPressed = false; btnDown.classList.remove('active'); ev.preventDefault?.(); };
  ...
  _setupMobileSticks() {
    ...
    const setupStick = (el, posKey) => {
      ...
      if (!knob) return;
      const onPointerDown = (e) => { try { el.setPointerCapture?.(e.pointerId); } catch (err) {} this.resetAutoRotate(); if (posKey === 'left') { if (this.mobile.leftTouchId == null) this.mobile.leftTouchId = e.pointerId; else return; } if (posKey === 'right') { if (this.mobile.rightTouchId == null) this.mobile.rightTouchId = e.pointerId; else return; } updateFromEvent(e); };
      const onPointerMove = (e) => { const id = e.pointerId; if (posKey === 'left') { if (this.mobile.leftTouchId === null) return; if (this.mobile.leftTouchId !== id) return; } if (posKey === 'right') { if (this.mobile.rightTouchId === null) return; if (this.mobile.rightTouchId !== id) return; } updateFromEvent(e); };
      const onPointerUp = (e) => { try { el.releasePointerCapture?.(e.pointerId); } catch (err) {} if (posKey === 'left') { this.mobile.leftTouchId = null; this.mobile.leftPos.x = 0; this.mobile.leftPos.y = 0; } if (posKey === 'right') { this.mobile.rightTouchId = null; this.mobile.rightPos.x = 0; this.mobile.rightPos.y = 0; } knob.style.transform = 'translate(0px, 0px)'; };
      const updateFromEvent = (e) => {
        ...
        if (posKey === 'left') { this.mobile.leftPos.x = nx; this.mobile.leftPos.y = ny; } else { this.mobile.rightPos.x = nx; this.mobile.rightPos.y = ny; }
      };
      ...
      el.addEventListener('touchstart', (ev) => {
        ...
        for (let i = 0; i < ev.changedTouches.length; i++) {
          ...
          if (t.clientX >= rect.left && t.clientX <= rect.right && t.clientY >= rect.top && t.clientY <= rect.bottom) {
            if (posKey === 'left') { if (this.mobile.leftTouchId == null) { this.mobile.leftTouchId = t.identifier; updateFromEvent(t); } } else { if (this.mobile.rightTouchId == null) { this.mobile.rightTouchId = t.identifier; updateFromEvent(t); } }
            break;
          }
        }
      }, { passive: false });
      ...
      el.addEventListener('touchend', (ev) => {
        ...
        for (let i = 0; i < ev.changedTouches.length; i++) {
          const t = ev.changedTouches[i];
          if (posKey === 'left' && t.identifier === this.mobile.leftTouchId) { this.mobile.leftTouchId = null; this.mobile.leftPos.x = 0; this.mobile.leftPos.y = 0; knob.style.transform = 'translate(0px, 0px)'; break; }
          if (posKey === 'right' && t.identifier === this.mobile.rightTouchId) { this.mobile.rightTouchId = null; this.mobile.rightPos.x = 0; this.mobile.rightPos.y = 0; knob.style.transform = 'translate(0px, 0px)'; break; }
        }
      }, { passive: false });
    };
  ...
  _getDist(t1, t2) { const dx = t1.clientX - t2.clientX; const dy = t1.clientY - t2.clientY; return Math.hypot(dx, dy); }
